...
public class AStarPathfinder { //этот класс реализует алгоритм поиска пути А*  в виде статического метода
    /**
     * Эта константа содержит максимальное ограничение стоимости пути. Если
     * конкретная путевая точка превышает этот предел стоимости, путевая точка отброшен
     **/
     ...
     *  местоположения указанной карты. Если путь найден, путевая точка
    ...
    public static Waypoint computePath(Map2D map)
    {
        ...
        {
            //Найдите «лучшую» путевую точку на данный момент.
    ...
    private static void takeNextStep(Waypoint currWP, AStarState state)
    {
        ...
        for (int y = loc.yCoord - 1; y <= loc.yCoord + 1; y++)
        {
            for (int x = loc.xCoord - 1; x <= loc.xCoord + 1; x++)
            {
                ...
                //Если это местоположение уже находится в "закрытом" наборе, затем перейдите к следующему местоположению.
                ...
                // Хорошо, мы обманываем и используем смету затрат для расчета фактического
                ...
                // путевая точка заменяет старую путевую точку только в том случае, если она дешевле чем старый.
